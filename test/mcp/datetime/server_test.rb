...
class MCP::DateTime::ServerTest < MCPTestHelper
  def setup
  ...
  def test_initialize_request
    ...
    assert_empty(response[:result][:capabilities][:tools])
  ...
  def test_get_date_info_specific_values
    ...
    # Test weekday logic
    if data["weekday_number"] == 0 || data["weekday_number"] == 6
  ...
  def test_all_datetime_fields_present
    ...
    required_fields = ["datetime", "timestamp", "year", "month", "day", "hour", "minute", "second", "weekday", "timezone"]
